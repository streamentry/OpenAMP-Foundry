...
_SCRIPT_PATH = _REPO_ROOT / "scripts" / "lab" / "validate_lab_data_return.py"
_spec = importlib.util.spec_from_file_location("_scripts_lab_validate_return", _SCRIPT_PATH)
_mod = importlib.util.module_from_spec(_spec)
assert _spec.loader is not None
_spec.loader.exec_module(_mod)
validate_lab_results_dir = _mod.validate_lab_results_dir
...
def test_cli_pass():
    with tempfile.TemporaryDirectory() as tmp:
        d = _make_valid_result(Path(tmp), "C1")
        r = subprocess.run(
            [sys.executable, "scripts/lab/validate_lab_data_return.py",
             "--results-dir", str(d)],
