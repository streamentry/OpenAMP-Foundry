...
from scripts.benchmarks.benchmark_simulation_baselines import (
    _auroc,
    _mean_eisenberg,
...
def test_cli_exit_code():
    result = subprocess.run(
        [sys.executable, "scripts/benchmarks/benchmark_simulation_baselines.py"],
...
def test_cli_writes_output_json(tmp_path):
    ...
    subprocess.run(
        [sys.executable, "scripts/benchmarks/benchmark_simulation_baselines.py", "--out", str(out)],
        capture_output=True, text=True,
        env={"PYTHONPATH": "src"},
    )
    assert out.exists()
