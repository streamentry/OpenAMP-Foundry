...
from scripts.benchmarks.benchmark_simulation_ablation import (
    _auroc,
...
def test_cli_amp_vs_decoy_exit_code():
    result = subprocess.run(
        [sys.executable, "scripts/benchmarks/benchmark_simulation_ablation.py", "--mode", "amp-vs-decoy"],
...
def test_cli_within_amp_exit_code():
    result = subprocess.run(
        [sys.executable, "scripts/benchmarks/benchmark_simulation_ablation.py", "--mode", "within-amp"],
        capture_output=True, text=True,
        env={"PYTHONPATH": "src"},
    )
    assert result.returncode == 3
...
def test_cli_writes_output_json(tmp_path):
    ...
    subprocess.run(
        [sys.executable, "scripts/benchmarks/benchmark_simulation_ablation.py",
