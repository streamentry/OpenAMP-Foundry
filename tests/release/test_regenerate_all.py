...
from scripts.release.regenerate_all import check_determinism, run_make, run_targets, sha256  # noqa: E402
...
class TestRegenerateAllScript:
    def test_help_flag_succeeds(self):
        result = subprocess.run(
            [sys.executable, "scripts/release/regenerate_all.py", "--help"],
    ...
    def test_skip_targets_passes_when_no_diff(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "scripts.release.regenerate_all.TRACKED",
            [str(snap)],
        )
        ...
        from scripts.release.regenerate_all import main
