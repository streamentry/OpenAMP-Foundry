...
**Synthetic or simulated results may not be used to raise any candidate's proof_ladder_level.**

Synthetic lab outcomes (generated by `generate-synthetic-lab-results` or any other script) are testing and calibration tools only. They do not constitute experimental evidence. A candidate's proof_ladder_level is determined exclusively by:
- the pipeline's computational scores (Levels 0-4);
- qualified external predictor results (Level 4, with caveats);
- expert human review (Level 5);
- real wet-lab assay results (Levels 6+).

This policy prevents circular reasoning: a simulator trained on computational scores cannot validate computational scores.
