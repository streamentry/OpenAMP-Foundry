...
>
> **Last updated:** 2026-07-09 (Phase G G10 — recalibration rollback plan — v0.5.88)
> **New in v0.5.88:** Recalibration rollback plan (G10) — `build_rollback_plan()` produces a structured `RollbackPlan` with 5 rollback triggers (RT-01 through RT-05) covering AUROC regression, calibration metric below threshold, safety filter bypass, cohort distribution shift, and unexplained weight delta. Each trigger has id, name, description, threshold, and severity. `RollbackStep` dataclass with step_number, action, responsible, detail, and dry_lab_only. `RollbackPlan` dataclass with plan_id, version, triggered_by, steps, notes, dry_lab_only, and to_dict(). 6 default rollback steps: halt, document, restore, verify, root-cause, and log. CLI (`openamp-foundry calibration-rollback-plan`) accepts `--plan-id`, `--version`, `--triggered-by`, `--notes`, `--out-json`, `--out-md`. JSON + Markdown output. Schema (`schemas/calibration_rollback_plan.schema.json`). `make calibration-rollback-plan` target. 15 tests. 3078 total. This completes Phase G (G1-G10 — calibration and active-learning rigor).
...
> **Pipeline version:** v0.5.88 (G10 — Phase G complete)
> **Branch:** main
