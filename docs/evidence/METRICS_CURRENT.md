...
>
> **Last updated:** 2026-07-10 (Phase N N2 — hypothesis outcome record — v0.9.5)
> **New in v0.9.5:** Hypothesis outcome record schema (N2 — Loop 134) — docs/evidence/HYPOTHESIS_OUTCOME_GUIDE.md with purpose, schema fields table (12 fields: outcome_id, registration_id, batch_id, pipeline_version, outcome_date, outcome_verdict, observed_metric_value, success_threshold_met, interpretation, deviation_from_plan, recorded_by, dry_lab_only), 4 valid outcome verdicts (confirmed, refuted, inconclusive, partially_confirmed), 6 validation rules, 4 warning conditions (threshold/verdict inconsistency, inconclusive no deviation, short interpretation), honest-use boundary. HypothesisOutcomeEntry dataclass (12 fields), HypothesisOutcomeResult dataclass (7 fields), validate_hypothesis_outcome() (6+ error checks: HOR- prefix, PRE- prefix, valid verdict, finite metric, non-empty interpretation<=500, non-empty recorded_by; 4 warning conditions: threshold/verdict mismatch, inconclusive no deviation, short interpretation), validate_hypothesis_outcome_dict() (11 required fields guard). CLI (openamp-foundry hypothesis-outcome-check) with --entry-json, --format text|json. make hypothesis-outcome-check target. 48 tests. Phase N N2: the pre-registration loop is now closed — outcomes are recorded and matched to their commitments.
> > **New in v0.9.4:** Pre-registration form schema (N1 — Loop 133) — docs/evidence/PRE_REGISTRATION_GUIDE.md with purpose, schema fields table (13 fields: registration_id, batch_id, pipeline_version, registration_date, primary_hypothesis, primary_outcome_metric, success_threshold, baseline_comparators, candidate_ids, assay_type, statistical_test, registered_by, dry_lab_only), 6 valid primary outcome metrics, 5 valid assay types, 10 validation rules, 4 warning conditions, honest-use boundary. PreRegistrationEntry dataclass (13 fields), PreRegistrationResult dataclass (6 fields), validate_pre_registration() (10+ error checks: PRE- prefix, non-empty hypothesis<=500 chars, valid outcome metric, finite success_threshold, >=1 baseline_comparators, >=1 candidate_ids, valid assay_type, non-empty statistical_test<=200 chars, non-empty registered_by, dry_lab_only must be True; 4 warning conditions: no random baseline, large candidate set >20, short hypothesis <50 chars, placeholder statistical test TBD/N/A/NA/none), validate_pre_registration_dict() (12 required fields guard). CLI (openamp-foundry pre-registration-check) with --entry-json, --format text|json. make pre-registration-check target. 62 tests. Phase N (Pre-registration & Baseline Honesty) started with N1.
