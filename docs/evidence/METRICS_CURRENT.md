...
>
> **Last updated:** 2026-07-10 (Phase M M4 — reviewer briefing package — v0.9.2)
> **New in v0.9.2:** Reviewer briefing package schema (M4 — Loop 131) — docs/evidence/REVIEWER_BRIEFING_GUIDE.md with purpose, schema fields table (11 fields), validation rules (7), warning conditions (4), honest-use boundary. src/openamp_foundry/evidence/reviewer_briefing_package.py with ReviewerBriefingEntry dataclass (11 fields: briefing_id, batch_id, pipeline_version, prepared_date, reviewer_name, candidate_count, artifact_ids, open_questions, scope_description, conflict_of_interest_declared, dry_lab_only=True), ReviewerBriefingResult dataclass (7 fields: briefing_id, batch_id, reviewer_name, passed, errors, warnings, dry_lab_only=True), MINIMUM_ARTIFACT_IDS (3), MAX_SCOPE_LENGTH (500), LONG_SCOPE_THRESHOLD (300), LARGE_BATCH_THRESHOLD (50), MAX_OPEN_QUESTIONS_WARNING (5), MINIMAL_ARTIFACT_WARNING_COUNT (3), validate_reviewer_briefing() (7 error checks: RBP- prefix, non-empty reviewer_name, candidate_count>=1, artifact_ids>=3, non-empty scope_description<=500 chars, conflict_of_interest_declared must be True, dry_lab_only must be True; 4 warning conditions: large batch, many open questions, long scope, minimal artifacts), validate_reviewer_briefing_dict() (10 required fields guard). CLI (openamp-foundry reviewer-briefing-check) with --entry-json, --format text|json. make reviewer-briefing-check target. 52 tests. Phase M M4: reviewer briefing packages now have a machine-validated schema for external auditor handoff.
