...
>
> **Last updated:** 2026-07-09 (Phase I I6 — adapter author validator — v0.6.4)
> **New in v0.6.4:** Adapter author validator (I6 — Loop 104) — `src/openamp_foundry/adapters/adapter_validator.py` with `AdapterDeclaration` dataclass (14 fields: adapter_id, adapter_version, mode, output_status, score_fields, uncertainty, warnings, failure_reason, release_status, ranking_effect, has_baseline_comparison, makes_network_calls, network_call_documented, dry_lab_only), `AdapterValidationResult` dataclass (5 fields: adapter_id, passed, errors, warnings_list, dry_lab_only), `validate_adapter_declaration()` (10 checks: non-empty adapter_id, non-empty adapter_version, valid mode in VALID_ADAPTER_MODES, valid output_status in VALID_OUTPUT_STATUSES, valid ranking_effect in VALID_RANKING_EFFECTS, valid release_status in VALID_RELEASE_STATUSES, dry_lab_only must be True, ranking_effect in proposed/active requires has_baseline_comparison=True, network calls require network_call_documented=True, uncertainty in 0.0-1.0 or None), `validate_adapter_dict()` for raw dict input with missing-fields guard. `VALID_ADAPTER_MODES` (4), `VALID_OUTPUT_STATUSES` (4), `VALID_RANKING_EFFECTS` (3), `VALID_RELEASE_STATUSES` (5), `REQUIRED_OUTPUT_CONTRACT_FIELDS` (10). All results carry dry_lab_only=True. CLI (`openamp-foundry adapter-check`) with `--adapter-json`, `--format text|json`. `make adapter-author-check` target. 31 tests. **3387 total.** External adapter authors can now validate their declarations against the ADAPTER_AUTHOR_GUIDE contract before submitting.
...
> **Pipeline version:** v0.6.4 (I6 — adapter author validator)
> **Branch:** main
