...
Current lab batch packs generated by `scripts/build_lab_batch_pack.py` include
`chain_of_custody.json` and `MANIFEST.json`. A receiver can run:

```bash
python scripts/build_lab_batch_pack.py --verify-pack outputs/lab_batch_pack.zip
```
