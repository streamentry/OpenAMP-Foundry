# Roadmap and Research

## Current direction

- [Roadmap](ROADMAP.md)
- [Next 100 PR map](NEXT_100_PR_MAP.md)
- [High-leverage tasks](../operations/HIGH_LEVERAGE_TASKS.md)
- [Current metrics](../evidence/METRICS_CURRENT.md)
- [Risk register](../trust/RISK_REGISTER.md)

## Strategy

- [Open infrastructure moat](OPEN_INFRASTRUCTURE_MOAT.md)
- [Open biotech stack](OPEN_BIOTECH_STACK.md)
- [Adoption strategy](ADOPTION_STRATEGY.md)
- [Adoption metrics](ADOPTION_METRICS.md)

## Research records

Wave plans, audits, recommendations, and postmortems preserve decisions under
the assumptions and data available at the time. Treat them as historical
evidence, not current policy or current metrics.

- [Historical 50-loop execution record](50_LOOP_PLAN.md)
- [Wave 0.5 baseline](WAVE_0_5_BASELINE.md)
- [Wave 0.5 novelty audit](WAVE_0_5_NOVELTY_AUDIT.md)
- [Wave 1 analysis plan](WAVE1_ANALYSIS_PLAN.md)
- [Wave 1 panel recommendation](WAVE_1_PANEL_RECOMMENDATION.md)
- [Wave 2 plan](WAVE2_PLAN.md)
- [Postmortems](../postmortems/)

Return to the [documentation home](../README.md).
