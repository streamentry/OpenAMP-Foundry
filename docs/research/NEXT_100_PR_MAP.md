...
|----|------|----------------|----------|
| T1 | Add batch traceability index schema (BTI-). | Per-batch index linking every candidate family in the batch to its evidence artifacts (BSP/SEG/ECC/STL/RSR/PQT); required_artifact_ids computed; completeness_grade (complete/partial/empty); enables cross-batch audit. | C |
| T2 | Add cross-batch audit summary schema (CBA2-). | Aggregates BTI records across all batches; counts families per grade; flags batches with low completeness; required for multi-batch meta-analysis. | C |
| T3 | Add batch evidence gap report schema (BEG-). | Per-batch listing of which artifact types are missing across all families; prioritized gap list; enables targeted evidence collection. | C |
| T4 | Add selection audit trail schema (SAT-). | Immutable ordered log of every selection/rejection decision with evidence_artifact_ids and rationale; enables full auditability from nomination to wet-lab shortlist. | C |
| T5 | Add pipeline completeness certificate schema (PCC-). | Top-level certificate asserting that all required evidence schemas are present for a given pipeline run; completeness_grade (A–D); closes Phase T. | C |
