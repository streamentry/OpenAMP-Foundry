...
| Loop | Bottleneck | Deliverable | Verification |
|------|-----------|-------------|-------------|
| 30 ✅ | No explicit virtual-assay scope document; stale BENCHMARKING.md; stale test count in METRICS_CURRENT.md; broken script ref in NOVELTY_AUDIT_GUIDE.md | `docs/evidence/VIRTUAL_ASSAY_SCOPE.md` — scope doc with modeled/NOT-modeled effects, uncertainty policy, ablation requirement, integration modes. Rewrote `docs/evidence/BENCHMARKING.md` with all 19 benchmarks. Fixed test count, script ref, scripts/__init__.py. | Scope doc reviewable; BENCHMARKING.md matches Makefile; 1843 tests |
...
| 39 ✅ | Virtual-assay benchmark report comparing simulation-augmented vs simulation-free ranking on the strict triage set | `docs/evidence/SIMULATION_BENCHMARK.md`: head-to-head report for simulation ablation, cheap-baseline comparison, and weighted-mode gate. v0.5.60. | Honest result: simulation did not improve ranking; weighted mode remains blocked. |
...
| Loop | Bottleneck | Deliverable | Verification |
|------|-----------|-------------|-------------|
| 40 ✅ | No assay-partner onboarding pack. A new CRO gets no README for how to run the panel | `docs/review/LAB_PARTNER_ONBOARDING.md`: panel manifest, synthesis instructions, assay protocol references, data return schema, positive/negative control sequences, safety notes, timeline, controls. v0.5.61 | A new CRO can read this and know exactly what to do |
...
| 45 ✅ | No chain-of-custody hashing for lab samples. Lab receives peptides but can't verify identity | `scripts/build_lab_batch_pack.py` now writes `chain_of_custody.json` + `MANIFEST.json` into the lab batch pack, including SHA-256 hashes for `panel.csv`, the ordered candidate list, each candidate sequence, and each evidence certificate. `--verify-pack` re-checks archive integrity end to end. v0.5.65 | Verification script passes on synthetic data and fails on tampered packs |
| 46 | No pre-registered analysis plan for Wave 1 results. What analysis runs when data comes back? | `docs/research/WAVE1_ANALYSIS_PLAN.md`: pre-registered statistical analysis (primary: MIC ≤ 32 hit rate; secondary: selectivity index; exploratory: per-family correlation). No post-hoc changes allowed | Plan is timestamped and locked before data return |
...
| 48 ✅ | No public-data release package. If results are publishable, what goes in the supplement? | `docs/review/PUBLICATION_PACK.md`: 12-section checklist covering pipeline code, input data, scores, benchmarks, panel, wet-lab results, calibration, reproducibility, negative results, safety, README, preprint prep. v0.5.68 | Pack is complete enough for a reviewer to reproduce all findings |
...
| 16 ✅ | Benchmark card stale — shows n=191 numbers only, missing all Phase 1 findings (Loops 9–15) | Rewrote `docs/evidence/BENCHMARK_CARD.md`: consolidated expanded benchmark, cluster-split-500, multi-negative, easy baseline, order-dependence, precision@k, rich selectivity, gate_triage, expert ablation (n=1000), updated known biases. Phase 1 exit criterion met. | Benchmark card is externally reviewable |
