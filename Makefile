...
reproducibility-manifest-check:
	.venv/bin/openamp-foundry reproducibility-manifest-check \
		--entry-json '{"manifest_id":"RPM-001","batch_id":"BATCH-001","pipeline_version":"0.8.5","run_date":"2026-07-09","python_version":"3.11.9","package_checksums":{"openamp_foundry":"0.8.4","numpy":"1.26.4","pandas":"2.2.2","scikit-learn":"1.4.2","torch":"2.3.0"},"data_checksums":{"data/amp_sequences.fasta":"abc123def456"},"random_seeds":{"ensemble_sampling":42},"hardware_summary":"Apple M3 Pro, 36 GB RAM","reviewer":"alice","dry_lab_only":true}' \
		--format text

candidate-summary-card-check:
	.venv/bin/openamp-foundry candidate-summary-card-check \
		--entry-json '{"card_id":"CRD-001","candidate_id":"AMP-001","batch_id":"BATCH-001","pipeline_version":"0.8.6","sequence":"KWKLFKKIEK","sequence_length":10,"evidence_level":4,"predicted_activity":"high_activity","safety_flags":[],"selection_rationale_id":"SEL-001","reviewer":"alice","dry_lab_only":true}' \
		--format text

multi-candidate-comparison-check:
	.venv/bin/openamp-foundry multi-candidate-comparison-check \
...
pipeline-decision-audit-check:
	.venv/bin/openamp-foundry pipeline-decision-audit-check \
...
claim-to-evidence-check:
	.venv/bin/openamp-foundry claim-to-evidence-check --entry-json '{"mapping_id":"CEM-001","batch_id":"BATCH-001","pipeline_version":"0.9.0","claim_text":"Test claim","claim_type":"activity_prediction","supporting_artifact_ids":["CERT-001","SEL-001"],"evidence_level":3,"pre_specified":true,"reviewer":"test@example.com","dry_lab_only":true}' --format text

score-decomposition-check:
	.venv/bin/openamp-foundry score-decomposition-check --entry-json '{"report_id":"SDR-001","batch_id":"BATCH-001","candidate_id":"AMP-042","pipeline_version":"0.9.1","composite_score":0.75,"component_scores":{"activity":0.8,"safety":0.7,"novelty":0.75},"component_weights":{"activity":0.5,"safety":0.3,"novelty":0.2},"scoring_method":"additive_weighted","score_range_min":0.0,"score_range_max":1.0,"reviewer":"test@example.com","dry_lab_only":true}' --format text

reviewer-briefing-check:
	.venv/bin/openamp-foundry reviewer-briefing-check --entry-json '{"briefing_id":"RBP-001","batch_id":"BATCH-001","pipeline_version":"0.9.2","prepared_date":"2026-07-10","reviewer_name":"Dr. Test Reviewer","candidate_count":10,"artifact_ids":["CERT-001","SEL-001","SDR-001"],"open_questions":["Is selection rationale adequate?"],"scope_description":"Review candidate evidence packages for BATCH-001.","conflict_of_interest_declared":true,"dry_lab_only":true}' --format text

audit-chain-check:
	.venv/bin/openamp-foundry audit-chain-check --entry-json '{"chain_id":"ACH-001","batch_id":"BATCH-001","pipeline_version":"0.9.3","audit_date":"2026-07-10","has_sequence_input":true,"has_benchmark_results":true,"has_filter_log":true,"has_score_decomposition":true,"has_selection_rationale":true,"has_evidence_certificate":true,"has_claim_mappings":true,"has_pipeline_decision_audit":true,"has_reviewer_briefing":true,"missing_links":[],"auditor":"test@example.com","dry_lab_only":true}' --format text

phase-ac-disconfirming-gate-check:
	PYTHONPATH=src $(PYTHON) -m openamp_foundry.cli phase-ac-disconfirming-gate-check --entry-json '{"acdg_id":"ACDG-001","pipeline_version":"demo","records":[{"dtr_id":"DTR-001","pipeline_version":"demo","claim_tested":"The pipeline adds signal beyond a cheap enemy.","test_type":"cheapest_explanation_check","test_description":"Compared the pipeline with a charge-only baseline on the frozen toy benchmark.","test_outcome":"not_refuted","evidence_summary":"The recorded challenge did not refute the claim.","limitations":["Toy benchmark only; this is not biological validation."],"created_at":"2026-07-15"}],"resolved_dtr_ids":[],"limitations":["The gate is a dry-lab review control, not biological proof."],"created_at":"2026-07-15"}' --format text
	@echo "Phase AC disconfirming-evidence gate check complete."
...
hypothesis-outcome-check:
	openamp-foundry hypothesis-outcome-check --entry-json '{"outcome_id":"HOR-001","registration_id":"PRE-001","batch_id":"BATCH-001","pipeline_version":"0.9.5","outcome_date":"2026-07-15","outcome_verdict":"confirmed","observed_metric_value":2.5,"success_threshold_met":true,"interpretation":"Selected candidates showed median MIC of 2.5 ug/mL, meeting the threshold. Random baseline showed 32 ug/mL. Hypothesis confirmed with strong signal vs random.","deviation_from_plan":"","recorded_by":"test@example.com","dry_lab_only":true}' --format text

baseline-comparison-check:
	openamp-foundry baseline-comparison-check --entry-json '{"manifest_id":"BCM-001","batch_id":"BATCH-001","pipeline_version":"0.9.6","comparison_date":"2026-07-10","metric_name":"hit_rate","pipeline_score":0.7,"baseline_scores":{"random_selection":0.1,"charge_matched":0.15},"pipeline_beats_all_baselines":true,"effect_size":0.55,"p_value":0.02,"comparison_direction":"higher_is_better","notes":"Pipeline beats random and charge-matched baselines.","reviewer":"test@example.com","dry_lab_only":true}' --format text

negative-result-check:
	openamp-foundry negative-result-check --entry-json '{"record_id":"NRR-001","batch_id":"BATCH-001","pipeline_version":"0.9.7","record_date":"2026-07-10","failure_category":"below_activity_threshold","failure_description":"Candidates did not meet MIC threshold in broth microdilution assay.","candidate_ids":["AMP-042"],"assay_type":"mic_assay","expected_outcome":"MIC <= 4 ug/mL","observed_outcome":"MIC = 128 ug/mL","hypothesis_impact":"Activity hypothesis refuted for these candidates. Pipeline review needed.","will_be_reported":true,"recorded_by":"test@example.com","dry_lab_only":true}' --format text
